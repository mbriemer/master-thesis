...
\begin{definition}[Neural network discriminator]
    ...
    Let $\theta_{train}$ be a collection of hyperparameters that specify the training of a neural network, such as a loss function, an intial guess, a number of training steps, a training algorithm, and a list of hyperparameters of the latter. %TODO extend/rewrite?
...
The classical way to turn the proabilities $\mathcal{D}(x)$ into an objective for the generator is the following: 
...
A discriminator that maximizes the cross-entropy loss therebay calculates the Jensen-Shannon divergence (plus a constant) for the generator to minimize.
